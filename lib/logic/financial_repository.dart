...
class FinancialRepository {
  Future<MonthlySummary> getMonthlySummary() async {
    ...
    final investmentsTotal = Sqflite.firstIntValue(await db.rawQuery('SELECT SUM(amount) FROM investments WHERE active=1')) ?? 0;
    
    // Net worth calc elements
    final npsTotal = Sqflite.firstIntValue(await db.rawQuery("SELECT SUM(amount) FROM retirement_contributions WHERE type = 'NPS'")) ?? 0;
    final pfTotal = Sqflite.firstIntValue(await db.rawQuery("SELECT SUM(amount) FROM retirement_contributions WHERE type = 'EPF'")) ?? 0;
    ...
    final creditCardDebt = Sqflite.firstIntValue(await db.rawQuery("SELECT SUM(statement_balance) FROM credit_cards")) ?? 0;
    final loansTotal = Sqflite.firstIntValue(await db.rawQuery("SELECT SUM(remaining_amount) FROM loans")) ?? 0;
    
    final netWorth = (investmentsTotal + npsTotal + pfTotal + otherRetirement) - (creditCardDebt + loansTotal);
  ...
  Future<List<Map<String, dynamic>>> getCategorySpending() async {
    final db = await AppDatabase.db;
    return await db.rawQuery('SELECT category, SUM(amount) as total FROM variable_expenses GROUP BY category ORDER BY total DESC');
  }
  ...
  Future<void> addEntry(String type, int amount, String category, String note, String date) async {
    final db = await AppDatabase.db;
    switch (type) {
      case 'Income': await db.insert('income_sources', {'source': category, 'amount': amount, 'date': date}); break;
      case 'Fixed': await db.insert('fixed_expenses', {'name': category, 'amount': amount, 'category': type, 'date': date}); break;
      case 'Subscription': await db.insert('subscriptions', {'name': category, 'amount': amount, 'active': 1, 'billing_date': '1', 'date': date}); break;
      case 'Investment': await db.insert('investments', {'name': category, 'amount': amount, 'active': 1, 'type': category, 'date': date}); break;
      default: await db.insert('variable_expenses', {'date': date, 'amount': amount, 'category': category, 'note': note});
    }
  }
  ...
  Future<void> addBudget(String category, int monthlyLimit) async {
    final db = await AppDatabase.db;
    await db.insert('budgets', {'category': category, 'monthly_limit': monthlyLimit});
  }

  Future<void> updateBudget(int id, int monthlyLimit) async {
    final db = await AppDatabase.db;
    await db.update('budgets', {'monthly_limit': monthlyLimit}, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<void> addCreditCard(String bank, int creditLimit, int statementBalance, int minDue, String dueDate) async {
    final db = await AppDatabase.db;
  ...
  Future<void> updateCreditCard(int id, String bank, int creditLimit, int statementBalance, int minDue, String dueDate) async {
    final db = await AppDatabase.db;
    await db.update(
      'credit_cards',
      {
  ...
  Future<void> updateGoal(int id, String name, int targetAmount, int currentAmount) async {
    final db = await AppDatabase.db;
    await db.update('saving_goals', {'name': name, 'target_amount': targetAmount, 'current_amount': currentAmount}, where: 'id = ?', whereArgs: [id]);
  }

  Future<void> addLoan(String name, String type, int total, int remaining, int emi, double rate, String due, String date) async {
    final db = await AppDatabase.db;
  ...
  Future<void> updateLoan(int id, String name, String type, int total, int remaining, int emi, double rate, String due) async {
    final db = await AppDatabase.db;
  ...
  Future<void> addSubscription(String name, int amount, String billingDate) async {
    final db = await AppDatabase.db;
    await db.insert('subscriptions', {'name': name, 'amount': amount, 'billing_date': billingDate, 'active': 1});
  }

  Future<void> updateEntry(String type, int id, Map<String, dynamic> values) async {
    final db = await AppDatabase.db;
    String table = "";
    switch (type) {
  ...
  Future<List<LedgerItem>> getMonthDetails(String month) async {
    ...
    final vars = await db.rawQuery("SELECT *, 'Variable' as entryType FROM variable_expenses WHERE substr(date, 1, 7) = ?", [month]);
    allItems.addAll(vars);
    final income = await db.rawQuery("SELECT *, 'Income' as entryType FROM income_sources WHERE substr(date, 1, 7) = ?", [month]);
    allItems.addAll(income);
    final fixed = await db.rawQuery("SELECT *, 'Fixed' as entryType FROM fixed_expenses WHERE substr(date, 1, 7) = ?", [month]);
    allItems.addAll(fixed);
    final invs = await db.rawQuery("SELECT *, 'Investment' as entryType FROM investments WHERE substr(date, 1, 7) = ?", [month]);
